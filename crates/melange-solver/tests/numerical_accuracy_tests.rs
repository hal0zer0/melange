...
"#;
    ...
    if output[1] > 1e-12 {
        ...
        assert!(
            ...
            "Decay ratio output[45]/output[1] = {:.4}, expected ~{:.4}",
            ratio,
            expected
        );
    }
...
fn test_diode_forward_voltage_shunt_clipper() {
    ...
    assert!(
        output[50] > output[5],
        "Output should increase during transient"
    );
...
fn test_diode_logarithmic_voltage_scaling() {
    ...
    assert!(
        (v5 - v1) < 0.2,
        "v_out(5V)-v_out(1V) = {:.4}, expected < 0.2",
        v5 - v1
    );
    ...
    assert!(
        inc < v2 * 0.5,
        "Increment 1->2V ({:.4}) should be << v_out(2V) ({:.4})",
        inc,
        v2
    );
}
...
fn test_multi_device_two_diodes_converges() {
    ...
    assert!(
        final_out > 0.0,
        "Final should be positive, got {:.6}",
        final_out
    );
...
fn test_multi_device_three_devices_converges() {
    ...
    assert!(
        final_out >= 0.0,
        "Final should be non-negative, got {:.6}",
        final_out
    );
    assert!(final_out < 1.0, "Final {:.4} should be < 1V", final_out);
}
...
fn test_bjt_common_emitter_bias_point() {
    ...
    assert!(
        final_out.abs() < 5.0,
        "Collector voltage {:.4}V should be < 5V",
        final_out
    );
}
...
fn test_purely_resistive_circuit_with_cap_stable() {
    ...
    assert!(
        (out_no[99] - out_with[99]).abs() < 0.01,
        "Steady states: no={:.6}, with={:.6}",
        out_no[99],
        out_with[99]
    );
...
fn test_dk_kernel_rejects_zero_sample_rate() {
    ...
    let mna = MnaSystem::from_netlist(&netlist).unwrap();
    assert!(
        DkKernel::from_mna(&mna, 0.0).is_err(),
...
fn test_dk_kernel_rejects_negative_sample_rate() {
    ...
    let mna = MnaSystem::from_netlist(&netlist).unwrap();
    assert!(
        DkKernel::from_mna(&mna, -44100.0).is_err(),
...
fn test_dk_kernel_rejects_nan_sample_rate() {
    ...
    let mna = MnaSystem::from_netlist(&netlist).unwrap();
    assert!(
        DkKernel::from_mna(&mna, f64::NAN).is_err(),
        "NaN sample rate should be rejected"
    );
}
