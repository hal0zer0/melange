...
fn runtime_v_source_stamped_in_all_rhs_rebuilds() {
    ...
    // via nodal_sub_path_override = FullLu (was a behavioral-dummy `B_frc frc 0 V={0}` routing lever).
    let full = nodal_code_with(CLIPPER_RUNTIME_V, |c| {
        c.nodal_sub_path_override = melange_solver::codegen::NodalSubPathOverride::FullLu
    });
    assert!(
