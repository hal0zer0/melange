...
use super::ir::{CircuitIR, DeviceParams, DeviceType, LuOp, PotentiometerIR};
...
impl RustEmitter {
    ...
    fn emit_device_models(&self, ir: &CircuitIR) -> Result<String, CodegenError> {
        ...
        if has_tube {
            // any_pentode guards the Rational (Derk §4.4) pentode emission block.
            // any_beam_tetrode guards the Exponential (DerkE §4.5) beam tetrode
            // block. For pure-triode circuits both are false, so the emitted code
            // is byte-identical to the pre-pentode baseline. A circuit with only
            // Rational pentodes (EL84/EL34/EF86) is byte-identical to phase 1a.
...
fn emit_schur_nr_limit_and_converge(
    code: &mut String,
    ir: &CircuitIR,
    dim: usize,
    indent: &str,
...
impl RustEmitter {
                ...
                match slot.device_type {
                    ...
                    DeviceType::Tube => {
                        ...
                        if tp.is_pentode() {
                            ...
                            // The helper family is selected per-slot by `screen_form`.
                            let s2 = s + 2;
                            let helper_suffix = match tp.screen_form {
                                crate::device_types::ScreenForm::Rational => "pentode",
                                crate::device_types::ScreenForm::Exponential => "beam_tetrode",
                            };
                            code.push_str(&format!(
    ...
    fn generate_schur_gauss_elim(code: &mut String, ir: &CircuitIR, dim: usize) {
        code.push_str(&format!(
            "        // Solve {dim}x{dim} system via inline Gaussian elimination\n"
        ));
...
impl RustEmitter {
    ...
    fn emit_nodal_state(&self, ir: &CircuitIR) -> String {
        ...
        // Schur complement matrices (precomputed S = A^{-1}, K = N_v*S*N_i, S_NI = S*N_i)
        ...
        if m > 0 {
            code.push_str("        self.k = K_DEFAULT;\n");
            code.push_str("        self.s_ni = S_NI_DEFAULT;\n");
        }
        ...
        if m > 0 {
            ...
            code.push_str("                    for a in 0..N {\n");
            code.push_str("                        let mut s_ni_aj = 0.0;\n");
            code.push_str("                        for b in 0..N { s_ni_aj += self.s_be[a][b] * N_I[b][j]; }\n");
            code.push_str("                        sum += N_V[i][a] * s_ni_aj;\n");
            code.push_str("                    }\n");
            code.push_str("                    self.k_be[i][j] = sum;\n");
            code.push_str("                }\n");
            code.push_str("            }\n");
            code.push_str("            for i in 0..N {\n");
            code.push_str("                for j in 0..M {\n");
    ...
    fn emit_nodal_schur_process_sample(ir: &CircuitIR) -> Result<String, CodegenError> {
        let n = ir.topology.n;
        let m = ir.topology.m;
        let n_nodes = if ir.topology.n_nodes > 0 {
            ir.topology.n_nodes
    ...
    fn emit_dk_device_eval_for_nodal_schur_indented(
        ...
        use super::ir::{DeviceParams, DeviceType};
        let s = slot.start_idx;
        let d = dev_num;
        ...
        for (dev_num, slot) in ir.device_slots.iter().enumerate() {
            for d in 0..slot.dimension {
                ...
                match (&slot.device_type, d) {
                    ...
                    (super::ir::DeviceType::Jfet, 0) | (super::ir::DeviceType::Mosfet, 0) => {
                        code.push_str(&format!(
                            "{indent}    let v_lim = fetlim(v_d{i} + dv{i}, v_d{i}, 0.0);\n"
                        ));
                    }
                    (super::ir::DeviceType::Jfet, _) => {
                        code.push_str(&format!(
                            "{indent}    let v_lim = fetlim(v_d{i} + dv{i}, v_d{i}, state.device_{dev_num}_vp);\n"
                        ));
                    }
                    (super::ir::DeviceType::Mosfet, _) => {
                        code.push_str(&format!(
                            "{indent}    let v_lim = fetlim(v_d{i} + dv{i}, v_d{i}, state.device_{dev_num}_vt);\n"
                        ));
                    }
                    ...
                    if tp.is_pentode() {
                        ...
                        code.push_str(&format!(
                            "{indent}{{ // Pentode {dev_num}\n\
                             ...
                             {indent}    let (ip_t, ig2_t, ig1_t, jac) = tube_evaluate_{helper_suffix}(vgk, vpk, vg2k, state.device_{dev_num}_mu, state.device_{dev_num}_ex, state.device_{dev_num}_kg1, DEVICE_{dev_num}_KG2, state.device_{dev_num}_kp, state.device_{dev_num}_kvb, DEVICE_{dev_num}_ALPHA_S, DEVICE_{dev_num}_A_FACTOR, DEVICE_{dev_num}_BETA_FACTOR, state.device_{dev_num}_ig_max, state.device_{dev_num}_vgk_onset);\n\
            ...
            match (&slot.device_type, &slot.params) {
                ...
                (DeviceType::Tube, DeviceParams::Tube(tp)) => {
                    ...
                    if tp.is_pentode() {
                        ...
                        code.push_str(&format!(
                            "{indent}i_nl[{s}] = tube_ip_{helper_suffix}(v_nl_final[{s}], v_nl_final[{s1}], v_nl_final[{s2}], state.device_{dev_num}_mu, state.device_{dev_num}_ex, state.device_{dev_num}_kg1, DEVICE_{dev_num}_KG2, state.device_{dev_num}_kp, state.device_{dev_num}_kvb, DEVICE_{dev_num}_ALPHA_S, DEVICE_{dev_num}_A_FACTOR, DEVICE_{dev_num}_BETA_FACTOR);\n\
                             {indent}i_nl[{s1}] = tube_is_{helper_suffix}(v_nl_final[{s}], v_nl_final[{s1}], v_nl_final[{s2}], state.device_{dev_num}_mu, state.device_{dev_num}_ex, state.device_{dev_num}_kg1, DEVICE_{dev_num}_KG2, state.device_{dev_num}_kp, state.device_{dev_num}_kvb, DEVICE_{dev_num}_ALPHA_S, DEVICE_{dev_num}_BETA_FACTOR);\n\
