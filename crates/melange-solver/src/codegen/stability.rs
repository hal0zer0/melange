...
///   plus float noise.
/// - `ρ > 0.999` AND `dominant_sign < 0`: Nyquist-marginal trap mode.
///   The dominant eigenvalue is real-negative near `-1`; trap has gain
///   magnitude exactly 1 at `z = -1`, so any seed from f64 round-off
///   in the per-sample matrix-vector multiplications persists
///   indefinitely. Catastrophic on high-gain cap-coupled cascades
///   (e.g. noyce-cascaded-triodes: 3× 12AX7, ρ = 0.9999, fs/2 limit
///   cycle of 28 mV at the output). The 0.999 threshold is safe
///   because the discriminator excludes circuits whose marginal
///   eigenvalue is positive (slow LF resonance — bilinear preserves
///   those exactly).
pub fn trap_needs_be(stability: TrapStability) -> bool {
    stability.rho > 1.002 || (stability.rho > 0.999 && stability.dominant_sign < 0.0)
}
...
mod tests {
    ...
    fn analyze_negative_identity_returns_unit_radius_negative() {
        ...
        assert!(trap_needs_be(r), "rho≈1 with negative dominant eigenvalue must trip auto-BE");
    }
