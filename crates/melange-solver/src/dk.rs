...
impl DkKernel {
    ...
    pub fn from_mna_augmented(mna: &MnaSystem, sample_rate: f64) -> Result<Self, DkError> {
        ...
        // Build A_neg = (2/T)*C - G, blanket-zeroing all algebraic rows n_nodes..n_aug.
        ...
        // Blanket-zero ALL algebraic constraint rows n_nodes..n_aug (VS, VCVS,
        // ideal-xfmr, Boyle op-amp internal, current-mode VCA internal+sense,
        // behavioral-V) — the previous per-type enumeration missed the VCA and
        // behavioral rows (same gap class as build_discretized_matrix, fixed
        // 2026-07-18). Inductor branch rows live at n_aug..n and keep their
        // L-in-C trapezoidal history. BJT transient internal nodes (appended
        // inside n..n_aug by expand_bjt_internal_nodes) are physical nodes and
        // are excluded defensively, mirroring build_discretized_matrix.
        {
            let mut is_bjt_internal = vec![false; n_aug];
            for bn in &mna.bjt_internal_nodes {
