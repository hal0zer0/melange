...
fn main() {
    ...
    println!(
        "N_aug={}, N_nodes={}, M={}",
        n_aug, kernel.n_nodes, kernel.m
    );
    println!(
        "Inductors: {} uncoupled, {} coupled pairs, {} transformer groups",
        mna.inductors.len(),
    ...
    let n_inductor_vars: usize = mna.inductors.len()
        ...
        + mna
            ...
            .sum::<usize>();
    println!(
        "N_nodal={} (N_aug + {} inductor variables)",
        n_aug + n_inductor_vars,
        n_inductor_vars
    );
    ...
    for (gi, g) in mna.transformer_groups.iter().enumerate() {
        println!(
            "  Group {}: {} windings: {:?}",
            gi, g.num_windings, g.winding_names
        );
    ...
    println!(
        "\nOutput node 'out': index {} (0-based: {})",
        out_node,
        out_node - 1
    );
    println!(
        "Input node 'in': index {} (0-based: {})",
        in_node,
        in_node - 1
    );
    ...
    for i in 0..num_samples {
        ...
        if i < 10 || (i % 48 == 0) {
            println!(
                "  sample {}: input={:.4}V, raw_out={:.4}V, dc_blocked={:.6}V",
                i, input, raw, out
            );
