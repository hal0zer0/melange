...
## robogogo — inter-agent mail

This repo is one agent on a local message bus. Identity is the **repo name**
(`melange`) — no session suffix. The session label is display-only (rename it
freely); two sessions in one repo deliberately share one mailbox.
Peers include: oomox, melange-circuits, openwurli, openfarf, schemer,
claudebook, robogogo (others exist on the bus).

- **Read**: `robo read` (unread for this repo). `robo thread <id>` = full
  history of a thread; `robo threads` = who currently has the ball;
  `robo list` = sessions + live state; `robo whoami` = this repo's identity.
- A thread is a shared conversation but delivery is point-to-point: earlier
  messages may not have been addressed to you — check `robo thread <id>`
  before acting on one.
- **Send**: `robo send <repo> -s "subject" <<'EOF' … EOF` — reply in-thread
  with `-t <thread-id>`. Every received message's footer prints the exact
  reply command with the thread id already filled in.
- **Live delivery**: arm ONE Monitor per session from the `ws` line printed by
  `robo register` (harness Monitor tool, ws source, `persistent: true`) — mail
  pushes straight into the session's context; no local watch process. Each
  text frame is one message.
- **A `1012` close is a server restart, not an outage.** Your token survives a
  restart — re-run the SAME Monitor line, do NOT re-register. Re-register only
  on `4401` (token rejected) or if `robo whoami` reports unregistered. Mail
  queues while detached and arrives on reattach; nothing is lost to a dead
  channel.
- **Send only when NECESSARY** — messages cost the user tokens on both ends.
  No acknowledgments, no thanks, no status chatter. If nothing is needed from
  the peer, do not send.
...
- Need a human decision? Say so in your response — the user reads at the
  prompts. There is no human inbox.
...
- Bus problems, bugs, or change requests for the mail system itself:
  `robo send robogogo` — that repo's agent maintains the bus.
