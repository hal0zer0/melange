...
| Op-amp false convergence on rail: `v[n_out] = ±VSAT` exactly every sample, forward-biased feedback diode drawing 10+ A at sample 0, `nr_max_iter_count = 0`, sub-Hz drift into exponential blow-up over tens of thousands of samples | `.model` VSAT mis-calibrated against the supply rails actually used in the netlist. An op-amp input that sits outside `[-VSAT, +VSAT]` gets DC-railed in simulation even when real hardware would not, and the strict-inequality `active_set_engaged` check misses the clamp-pinned node. | **FIXED 2026-04-17.** Two-part: (a) `.model OA_TL074` corrected to `VSAT=13.5` for ±15 V supplies (was `VSAT=11`, appropriate for ±12 V) — the root cause on 4kbuscomp; (b) residual check extended from BoyleDiodes to `ActiveSetBe \| ActiveSet` in `nodal_emitter.rs` to catch the same bug class on other topologies. 4kbuscomp at `d=5 s, amp ∈ {0.01, 0.1, 0.5}`: zero NR/BE events, faster than realtime. See `docs/aidocs/DEBUGGING.md` "ActiveSetBe Chord-NR False Convergence". |
...
- Bus compressor (4kbuscomp): 12 op-amps, 2 VCAs, 6 diodes, 2 pots, 2 switches
  ...
  - **Full circuit stable end-to-end** (2026-04-17). DC OP converges, transient stable at `d=5 s, amp ∈ {0.01, 0.1, 0.5}` with zero NR max-iter hits and zero BE fallbacks, faster than realtime. Peaks scale linearly below threshold (0.01→0.0056, 0.1→0.0562) and show compressor action above (0.5→0.28).
  - Root cause of the long-standing precision-rectifier instability was a one-line `.model` error in the netlist: `OA_TL074 VSAT=11` (appropriate for ±12 V supplies) with circuit supplies at ±15 V. On ±15 V a real TL074 saturates at ~±13.5 V and U8/U9's `n_plus = vee12 = -12 V` is inside the output range; in simulation with VSAT=11 it was permanently DC-railed, triggering the forward-biased feedback-diode false convergence described in `docs/aidocs/DEBUGGING.md`. Fix: `.model OA_TL074 OA(... VSAT=13.5 ...)`.
...
- SSL bus compressor (full circuit stable end-to-end 2026-04-17; root-caused a long-standing "solver bug" to a `.model OA_TL074 VSAT=11` mis-calibration against the ±15 V supplies, fixed in the netlist to `VSAT=13.5`; residual-check extension to `ActiveSetBe \| ActiveSet` kept as general robustness)
