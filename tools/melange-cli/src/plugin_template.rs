...
Audio plugin generated by [Melange](https://github.com/hal0zer0/melange) from a SPICE circuit netlist.
...
mod tests {
    ...
    #[test]
    fn cargo_toml_declares_own_workspace() {
        // The generated project must carry an empty [workspace] table so it
        // builds standalone even when generated inside another cargo workspace
        // (e.g. the melange repo, per the README quick-start). Without it,
        // `cargo build` errors: "current package believes it's in a workspace
        // when it's not".
        let toml = test_generate_cargo_toml("test");
        assert!(
            toml.contains("[workspace]"),
            "generated Cargo.toml must declare its own [workspace] root"
        );
    }
