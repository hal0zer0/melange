...
/// - `// melange: <v> (<c>) exe fnv1a64:<h>`  → `// melange: <version> (<commit>)`
///   (the whole line is replaced, so the exe hash on it is dropped too)
/// - the `// provenance: {...}` JSON: the `"melange"`, `"commit"` and
///   `"exe_fnv1a64"` values are replaced with placeholders; every other
///   (deterministic, DSP-contract) field is preserved verbatim.
...
mod provenance_norm_tests {
    ...
    fn masks_melange_and_provenance_lines_only() {
        let code = "// Generated by melange-solver\n\
                    // melange: 0.1.0 (e2f6d62) exe fnv1a64:8b62d950c3c6f88b\n\
                    // Circuit: \"X\"\n\
                    // Build: integration=trapezoidal, dc-block=on\n\
                    // provenance: {\"melange\":\"0.1.0\",\"commit\":\"e2f6d62\",\"exe_fnv1a64\":\"8b62d950c3c6f88b\",\"dc_block\":true}\n\
                    pub const N: usize = 2;\n";
        let out = normalize_provenance_header(code);
        assert!(out.contains("// melange: <version> (<commit>)"));
        // The exe hash on the `// melange:` line is dropped with the wholesale
        // line replacement.
        assert!(!out.contains("8b62d950c3c6f88b"));
        assert!(out.contains("\"melange\":\"<version>\""));
        assert!(out.contains("\"commit\":\"<commit>\""));
        assert!(out.contains("\"exe_fnv1a64\":\"<exe>\""));
        ...
        // Two builds with different version/commit AND different exe hash
        // normalize identically — a clean rebuild does not read as a codegen
        // change.
        let code2 = code
            .replace("0.1.0 (e2f6d62)", "0.2.0 (abc1234)")
            .replace(
                "\"melange\":\"0.1.0\",\"commit\":\"e2f6d62\"",
                "\"melange\":\"0.2.0\",\"commit\":\"abc1234\"",
            )
            .replace("8b62d950c3c6f88b", "55c248b088a69bb6");
