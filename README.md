...
> **Status: early alpha.** Several classic circuits compile, run, and agree with ngspice at the SPICE level (Pultec-class EQ, Wurlitzer 200A, tweed preamp, Tube Screamer). One — the Wurlitzer 200A preamp — is additionally checked against real-hardware measurements. "Validated" means different things for different circuits; each claim's reach is scoped in [Validation & Verification](docs/VALIDATION.md) and the [circuits table](#circuits).

> **"A netlist-to-plugin factory for any audio circuit? Too good to be true."** Fair — so here are the receipts. **[Validation & Verification →](docs/VALIDATION.md)** lays out exactly what is proven and what isn't: the solver's numerical correctness and the device models are verified to machine precision *without* leaning on another simulator (Tellegen power-balance, convergence-order, canonical device equations), while circuit *fidelity* to real hardware is honestly scoped — anchored to measured hardware for one circuit so far, with every other claim's reach stated plainly. The candor is deliberate; skepticism is the correct starting point.
