...
Performance (measured on an AMD Ryzen 9 7950X, single core, noiseless, `-C target-cpu=x86-64-v3`, via `tools/perf-harness/bench.sh`; host-dependent): light nonlinear circuits run into the hundreds of × realtime (a single 12AX7 stage ~230×), typical multi-device audio circuits ~30–65× (Wurlitzer preamp ~56×, tweed amp ~29×), and the heaviest validated circuits ~9–28× — the Pultec EQP-1A (nodal full-LU with chord + sparse LU, N=46, M=8) at ~28×, an SSL-class bus compressor at ~9×.
