...
| Click on every buffer when plugin drives `set_pot_N` from an envelope follower (sag, bias modulation, LFO of a bias resistor) | Envelope deltas cross the 20% warm DC-OP re-init gate on every ballistic attack/release. The gate is correct for user knob drags — at envelope rates each reseed snaps `v_prev`/`i_nl_prev` mid-signal, audible as a click | Declare the resistor with `.runtime R Rname min max as field` instead of `.pot R`. Codegen emits `set_runtime_R_<field>(r)` that clamps + marks dirty WITHOUT the DC-OP snap. Plugin drives the setter directly (no nih-plug knob). Shipped 2026-04-19 (commit `fa8474d`). See `DYNAMIC_PARAMS.md` "Difference from `.pot R`" |
