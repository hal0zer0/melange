...
- **`.oversampling {1|2|4}` netlist directive (2026-09-05)**: a deck declares its recommended oversampling factor to control aliasing from nonlinear distortion products. It is an accuracy **minimum/recommendation, not a mandate** — rate costs CPU/latency (the plugin author's call). Resolution on compile/simulate/analyze: an explicit `--oversampling` always wins (even when lower — logs a `log::warn!`), else the deck value, else 1. **`validate` does NOT read the directive** — it takes `--oversampling {1|2|4}` explicitly (default 1) and reports what it was asked to measure; the reference is compensated with the same half-band round trip (see OVERSAMPLING.md § Validating an oversampled build). Stripped for ngspice via `MELANGE_ONLY_DIRECTIVES`.
