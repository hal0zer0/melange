...
- **wurli-power-amp residual** (raised by melange-circuits 2026-07-25, after the auto-BE router-corroboration fix `b0dcb27` closed the timeout/explosion bug): ~10 dB past clipping (input amplitude 1.0, rails ±22 V) output still reaches 353 V, and `nr_max_iter_count` is high (1000s of hits per 0.5 s) even at input levels where output is correct. Not yet triaged; likely a distinct NR-robustness question (possibly related to the existing "genuine nodal full-LU NR-robustness bug at ≥3mV Class-AB crossover" open item), not an integrator-choice bug. No ticket opened yet — awaiting user decision on priority.
...
- **`melange simulate`/`analyze --noise` is a silent no-op** (found by melange-circuits 2026-07-25, `melange-circuits/local-docs/melange-bug-simulate-noise-noop-2026-07-25.md`): CLI plumbing + codegen are correct (noise machinery IS emitted with `--noise full`), but the generated `noise_enabled` runtime master switch defaults `false` and nothing in `simulate`/`analyze` calls `set_noise_enabled(true)`. Output is byte-identical and seed-invariant to noise-off — a measurement that looks like a clean result but is actually just silently disabled. Suggested fix (not yet applied): call `set_noise_enabled(true)` in the simulate/analyze codegen-runner paths when `noise_mode != NoiseMode::Off`. Blocks melange-circuits' Noyce-palette noise characterization and an oomox absolute-dBFS re-baseline. Small, well-scoped fix — good candidate for next session.
