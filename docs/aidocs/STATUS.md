...
- **Pentode / beam tetrode**: Reefman equations with two screen-current forms: **Rational** (Derk §4.4, `1/(1+β·Vp)`) for true pentodes, **Exponential** (DerkE §4.5, `exp(-(β·Vp)^{3/2})`) for beam tetrodes. 9 fitted params per tube (μ, Ex, Kg1, Kg2, Kp, Kvb, αs, A, β) plus `ScreenForm` discriminator. Catalog (all from Reefman TubeLib.inc 2016): EL84/6BQ5, EL34/6CA7, EF86/6267 (Rational, `-P` suffix aliases); 6L6GC/5881, 6V6GT (Exponential, `-T` suffix aliases). New element prefix `P` (`P n_plate n_grid n_cathode n_screen [n_suppressor] model`) and `VP` model token. Phase 1b adds grid-off FA reduction; phase 1c adds variable-mu for varimu compressors.
- **Not implemented**: temperature coefficients on resistors (TC1/TC2), noise models, KT88/6550 pentode mode (phase 1a.2 — Reefman has no published fit), remote-cutoff variable-mu tubes (phase 1c)
