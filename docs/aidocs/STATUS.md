...
- **Not implemented**: temperature coefficients on resistors (TC1/TC2), op-amp en/in noise (Phase 4), pentode partition noise (Phase 5), 6386/6BA6/6BC8 datasheet fits for varimu compressors (phase 1d deferred). Thermal/shot/flicker noise on R + diode/BJT/JFET/MOSFET/tube junctions ARE implemented — see "Circuit Noise" under Feature Inventory.
...
### Circuit Noise (Phases 1–3, opt-in via `--noise {thermal|shot|full}`)
- **Thermal (Phase 1)**: Johnson-Nyquist on every fixed R + every dynamic R (`.pot`/`.wiper`/`.runtime R`/`.switch` R). Per-sample two-draw stamp `i_n = w_new + w_prev` with `w_new = (scale/2)·sqrt(1/R)·g` — zeros the Nyquist bin so resistor-only nodes don't accumulate (FIXED 2026-04-24 `a5dff8c`). DK path; nodal still single-draw pending a circuit that needs the fix.
- **Shot (Phase 2)**: per-junction `sqrt(4·q·|I_prev|·fs)` using one-sample-lagged `state.i_nl_prev`. Diode 1 src, BJT 2 (1 when forward-active reduced), JFET/MOSFET 1, Tube 1. VCA/op-amp skipped.
- **Flicker (Phase 3)**: per-junction `kellett_pink(sqrt(4·KF·|I_prev|^AF·fs)·g)` via Paul Kellett 7-pole pink filter (×0.11 unity-power-gain normalization). Opt-in per device via `.model NAME TYPE(KF=… AF=…)`; KF=0 (default) → byte-identical to no-flicker codegen.
- Runtime: `set_noise_enabled(bool)`, `set_noise_gain`, `set_thermal_gain`/`set_shot_gain`/`set_flicker_gain`, `set_temperature_k(K)` (290 K default; cold gear is quieter), `set_seed(u64)` (0 → entropy, nonzero → deterministic). Salted streams (`NOISE_SHOT_SALT`, `NOISE_FLICKER_SALT`) so thermal/shot/flicker never share a prefix under one master seed.
- Calibration validated by kTC theorem (`tests/noise_psd_validation.rs`): `V²_rms = k_B·T/C` ±15% on a 10 kΩ / 100 nF RC. Nyquist regression: `thermal_noise_no_nyquist_artifact_on_resistor_only_output_node` asserts lag-1 > -0.5 AND RMS < 100 µV on a diode + series-R circuit. Full reference: [NOISE.md](NOISE.md). Pending: Step 6 op-amp en/in, Step 7 pentode partition.
