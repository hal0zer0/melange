...
- **Skip warmup with `recompute_dc_op()`.** Pass `--emit-dc-op-recompute` to `melange compile` and the generated `CircuitState` gains a `recompute_dc_op(&mut self)` method. Set jittered pot/switch values, call it once, and the solver jumps directly to the new equilibrium in tens of microseconds — no silent loop required. Not audio-thread safe; call from plugin init or parameter-change callbacks. DK-path circuits (tube preamps, op-amp clippers) get the full NR solve today; nodal full-LU circuits (pultec, 4kbuscomp, VCR ALC) ship a stub that bumps `diag_nr_max_iter_count` and returns — watch the counter and fall back to the warmup loop until the full nodal body ships. See `docs/aidocs/DC_OP.md` "Runtime DC OP recompute" for the full contract.
